...
allowed-tools: Read, Bash(ls:*), Bash(gh issue create:*), Bash(gh label list:*)
---
...
Auditor Agent SENGAJA tidak boleh create ticket sendiri (lihat `.claude/agents/caf-auditor.md`
§ Scope: "itu keputusan manusia, bukan agent ini"). Command ini adalah satu-satunya jalur resmi dari audit-report.md ke GitHub Issues — jangan buat jalur lain.

## Argumen

`$ARGUMENTS` — opsional, path ke file audit spesifik (mis.
`.caf/audits/2026-07-10/audit-report.md` dari agent `auditor`, atau
`.caf/audits/2026-07-10/audit-report-{scope-slug}.md` dari
`/caf-audit-scan`). Kalau kosong, cari folder tanggal terbaru di `.caf/audits/`.

## Langkah

### 1. Temukan dan baca file audit

- Kalau `$ARGUMENTS` kosong: `ls -t .caf/audits/` untuk cari folder tanggal
  terbaru, lalu `ls .caf/audits/<DATE>/` untuk lihat semua file
...
- Parse SEMUA section "## Temuan Prioritas" di file yang dipilih — file bisa
  berisi lebih dari satu section ini kalau `/caf-audit-scan` di-run beberapa kali
...
### 3. Create GitHub issue (hanya untuk yang di-approve)

Default: pakai `gh issue create` (CLI, sudah pasti tersedia tanpa setup MCP tambahan).
Kalau sesi ini punya MCP GitHub Issues terpasang, boleh pakai itu sebagai gantinya — tapi
verifikasi dulu nama tool-nya sebelum pakai, jangan asumsi.

Title: judul temuan (ringkas, actionable — bukan copy-paste deskripsi panjang)
Body: format berikut, isi dari audit-report.md (atau versi editan):
```
## Sumber
Auditor Agent scan <tanggal audit>, file: `.caf/audits/<tanggal>/audit-report.md`
...
Jalankan:
```bash
gh issue create --title "<judul>" --body "<body di atas>" --label "<label>"
```
- Cek label yang tersedia dulu dengan `gh label list` — kalau label yang dimaksud belum
  ada di repo, buat dulu manual atau tanya user (jangan asumsi ada).
- Label mapping dari kategori:
  - `BUG` → label `bug` (+ tandai prioritas di title/body kalau repo punya konvensi
    lain, mis. issue template)
  - `PERFORMANCE` → label `performance`
  - `TECH_DEBT` → label `tech-debt`
  - `COVERAGE` → label `test-coverage`
- Simpan issue number + URL yang dikembalikan `gh issue create`.
...
- JANGAN buat folder `.caf/tasks/<TICKET-ID>/` — itu domain Planner Agent setelah
  ticket masuk pipeline normal, bukan tanggung jawab command ini.
- Kalau `gh issue create` gagal (auth `gh auth status`, rate limit, dll) di tengah
  proses, STOP dan laporkan progress sejauh mana (jangan retry diam-diam, jangan
  lanjut ke temuan berikutnya seolah semua baik-baik saja).
