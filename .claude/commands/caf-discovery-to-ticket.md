...
allowed-tools: Read, Write, Bash(ls:*), Bash(gh issue create:*), Bash(gh label list:*)
---
...
keputusan manusia. Command ini adalah satu-satunya jalur resmi dari dokumen discovery ke GitHub Issues — jangan buat jalur lain.
...
### 4. Create GitHub issue (hanya untuk yang di-approve)

Default: pakai `gh issue create` (CLI, sudah pasti tersedia tanpa setup MCP tambahan). Kalau
sesi ini punya MCP GitHub Issues terpasang, boleh pakai itu sebagai gantinya — tapi verifikasi
dulu nama tool-nya sebelum pakai, jangan asumsi.

Title: judul ticket yang sudah dikonfirmasi user di langkah 3 (ringkas, actionable).
Body: format berikut, isi dari prd.md/flow.md (atau versi editan):
```
## Sumber
...
```
- Cek label yang tersedia dulu dengan `gh label list` — kalau label yang dimaksud belum ada
  di repo, buat dulu manual atau tanya user (jangan asumsi ada). Default masuk akal: label
  `feature` atau `enhancement`, tergantung mana yang ada di repo.
- Simpan issue number + URL yang dikembalikan `gh issue create` — dipakai di langkah 5.
...
- Kalau `gh issue create` gagal (auth `gh auth status`, rate limit, dll) di tengah proses,
  STOP dan laporkan progress sejauh mana, lalu tetap tulis `handoff.md` untuk ticket yang
  SUDAH berhasil dibuat (jangan retry diam-diam, jangan lanjut ke item berikutnya seolah
  semua baik-baik saja, dan jangan biarkan ticket yang sudah jadi tidak tercatat).
