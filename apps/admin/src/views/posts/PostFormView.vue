<template>
  <div class="p-6 max-w-4xl mx-auto">
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-2xl font-bold">{{ isEdit ? 'Edit Post' : `New ${typeLabel}` }}</h1>
    ...
    <form @submit.prevent="handleSubmit" class="space-y-6 bg-white rounded-xl border p-6">
      ...
      <div>
        ...
        <select v-model="form.type" :disabled="isEdit" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-gray-50">
...
const typeParam = computed(() => route.params.type as string || 'article');
...
onMounted(async () => {
  if (isEdit.value) {
    try {
      const { data } = await import('@/lib/api').then(m => m.default.get(`/admin/posts`));
