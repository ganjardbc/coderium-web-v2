...
- ✍️ **AI Content Generation**: `POST /admin/ai-content/generate` (LLM OpenAI-compatible + web search, cari & rewrite artikel trending jadi draft Bahasa Indonesia) dan `POST /admin/ai-content/cover` (download+reupload cover eksternal jadi URL internal) — backend di `apps/api` (ticket 24). UI `apps/admin` untuk memicu generate & commit ke `POST /admin/posts` adalah ticket terpisah (ticket 25, belum dikerjakan). Lihat `docs/api/api-contract.md` (`# AI Content API`) untuk kontrak endpoint lengkap.
- ✨ **AI Agent (generate artikel AI)**: modul admin baru `/ai-agent` untuk generate draft artikel via LLM (grid card, tipe konten lain "Segera Hadir"), preview hasil sebelum commit, lalu publish/simpan sebagai draft — reuse endpoint `POST /admin/posts` existing. Backend di `apps/api` (ticket 24, modul `ai-content`, PR terpisah), UI di `apps/admin` (ticket 25). Lihat `docs/api/api-contract.md` (`# AI Content Integration`) untuk kontrak endpoint.
